...
import net.walksanator.uxncraft.regs.BlockEntityTypes
...
class TerminalBlock(settings: Settings) : BlockWithEntity(settings), BlockEntityTicker<TerminalEntity> {
    ...
    override fun onUse(state: BlockState, world: World, pos: BlockPos, player: PlayerEntity, hand: Hand, hit: BlockHitResult): ActionResult {
        ...
        if (world.isClient) Uxncraft.proxy.openTerminalScreen(te)
        return SUCCESS
    }
    ...
    override fun <T : BlockEntity?> getTicker(
        ...
        return if (!world.isClient && type == BlockEntityTypes.terminal) {
...
class TerminalEntity(pos: BlockPos, state: BlockState) : BlockEntity(BlockEntityTypes.terminal, pos, state) {

    val screen = ByteArray(80 * 50) { 0x20 }
    val charset: ByteArray = Uxncraft.resources!!.charset.clone()
    val kb = ByteArray(16)
